...
def make_OMscore(LL_sam, state, quantile=0.878):
    # 리뷰가중평점
    ...
    LL_sam["weighted_rating"] = LL_sam.apply(
        (
            lambda x: (
                x["rating_count"] / (x["rating_count"] + m) * x["rating_average"]
            )
            + (m / (m + x["rating_count"]) * C)
        ),
        axis=1,
    )
    ...
    LL_sam["distance"] = [
        haversine(state, list((LL_sam[["Latitude", "Longtitude"]].iloc[i])))
        for i in range(len(LL_sam))
    ]
    ...
    # 어디가맨 지수
    LL_sam["OMscore"] = (
        LL_sam["distance_log"] * 0.415 + LL_sam["weighted_rating"] * 0.585
    )

    # 어디가맨 지수 상위 10개 식당
    top10 = LL_sam[
        ["name", "address", "OMscore", "Latitude", "Longtitude", "upjong"]
    ].sort_values("OMscore", ascending=False)[:10]
    ...
    dic["upjong"] = list(top10["upjong"].values)
